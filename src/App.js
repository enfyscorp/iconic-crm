...
// ─── DEFAULT BOOTSTRAP DATA (written to storage ONCE if storage is empty) ─
...
export default function App() {
  ...
  // ── Bootstrap storage on first load ──────────────────────────────────────
  useEffect(() => {
    (async () => {
  ...
  // Sync setActivityLogs wrapper after state changes
  ...
  const ForgotModal=()=>{
    ...
    return(
      <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
        <div className="bg-slate-950 border border-slate-800 w-full max-w-md rounded-2xl p-6 space-y-5 shadow-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center"><KeyRound className="h-5 w-5 text-orange-400"/></div>
              <div><h3 className="text-sm font-black text-white uppercase tracking-wide">Admin Credentials</h3><p className="text-[10px] text-slate-500 mt-0.5">Only visible to authorized personnel</p></div>
            </div>
            <button onClick={()=>setShowForgotModal(false)} className="text-slate-500 hover:text-white"><X className="h-5 w-5"/></button>
          </div>
          <div className="bg-amber-950/30 border border-amber-500/30 rounded-xl p-3 flex items-start gap-2">
            <AlertTriangle className="h-4 w-4 text-amber-400 mt-0.5 flex-shrink-0"/>
            <p className="text-[11px] text-amber-300 font-medium">This panel is for Admin account recovery only. All other user passwords must be reset by an Admin from the System Control Hub.</p>
          </div>
          <div className="space-y-3">
            {adminAccounts.map(u=>(
              <div key={u.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 rounded-lg bg-orange-600 font-black text-xs flex items-center justify-center text-white">{u.avatar}</div>
                  <p className="font-black text-white text-sm">{u.name}</p>
                  <span className="text-[9px] bg-orange-500/10 text-orange-400 font-black px-2 py-0.5 rounded uppercase border border-orange-500/20">Admin</span>
                </div>
                <div className="space-y-1 pl-9">
                  <div className="flex items-center gap-2 text-xs"><span className="text-slate-500 w-20 font-bold">Username:</span><code className="text-orange-400 font-mono bg-slate-950 px-2 py-0.5 rounded border border-slate-800">{u.email}</code></div>
                  <div className="flex items-center gap-2 text-xs"><span className="text-slate-500 w-20 font-bold">Password:</span><code className="text-emerald-400 font-mono bg-slate-950 px-2 py-0.5 rounded border border-slate-800">{u.pass}</code></div>
                </div>
              </div>
            ))}
          </div>
          <button onClick={()=>setShowForgotModal(false)} className="w-full bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-white font-black py-2.5 rounded-xl text-xs uppercase tracking-wider transition-colors">Close</button>
        </div>
      </div>
    );
  };
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans antialiased text-slate-200">
        ...
        <div className="mt-6 sm:mx-auto w-full max-w-md px-4">
          <div className="bg-slate-950 py-8 px-6 border border-slate-800 rounded-2xl shadow-2xl space-y-6">
            ...
            {/* Forgot credentials — Admin only (shows modal with admin accounts) */}
            <div className="pt-2 border-t border-slate-900 flex justify-center">
              <button onClick={()=>setShowForgotModal(true)} className="flex items-center gap-1.5 text-[11px] text-slate-500 hover:text-orange-400 transition-colors font-bold uppercase tracking-wide">
                <HelpCircle className="h-3.5 w-3.5"/> Admin — Forgot Credentials?
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateLead} className="space-y-4 text-xs">
              ...
              {/* ── ASSIGNED TO (Admin only in lead creation) ── */}
              {currentUser.role==="Admin"&&(
