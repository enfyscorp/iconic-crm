...
export default function App() {
  ...
  const handlePhoneInputChange = (inputRawValue, isAlternateField = false) => {
    ...
    if (isAlternateField) {
      ...
      if (fullyCleanDigits.length >= 10) {
        ...
        if (matchedDuplicate) { setDuplicateConflictRecord(matchedDuplicate); return; }
      }
  ...
  const handleInlineMilestoneStatusChange = (leadId, targetStatus) => {
    const updatedHistoryLog = { date: TODAY_STR, by: currentUser.name, action: `Transformed active pipeline milestone to state tracked tier [${targetStatus}].` };
    setLeads(leads.map(l => l.id === leadId ? { ...l, status: targetStatus, history: [updatedHistoryLog, ...l.history] } : l));
    if (selectedLead && selectedLead.id === leadId) {
      setSelectedLead(prev => ({ ...prev, status: targetStatus, history: [updatedHistoryLog, ...prev.history] }));
    }
  ...
  const commitTentativeWalkthroughPlan = (e) => {
    ...
    const plannedHistoryLog = { date: TODAY_STR, by: currentUser.name, action: `[SITE VISIT PLANNED ALERT]: Walkthrough itinerary drafted. Scheduled tentative review date target set to: ${tentativeWalkthroughDateInput}.` };
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, status: "Site Visit Planned", siteVisitTentativeDate: tentativeWalkthroughDateInput, history: [plannedHistoryLog, ...l.history] } : l));
    setSelectedLead(prev => ({ ...prev, status: "Site Visit Planned", siteVisitTentativeDate: tentativeWalkthroughDateInput, history: [plannedHistoryLog, ...prev.history] }));
    setTentativeWalkthroughDateInput("");
    triggerToastAlert("Tentative date saved.");
  };

  const executeDataExportSequence = (formatType) => {
    if (processedLeads.length === 0) { triggerToastAlert("No scoped logs available to compile."); return; }
    ...
    const contentRows = processedLeads.map(l => [l.id, l.name, l.phone, l.email || "N/A", l.project, `${l.budget}L`, l.source, l.assignedTo, l.status, l.dateCreated]);
    let rawOutputBuffer = "";
    ...
    if (formatType === "csv" || formatType === "excel") {
      const formatCSVCell = (val) => {
        ...
        return textVal.includes(",") || textVal.includes("\n") || textVal.includes('"') ? `"${textVal.replace(/"/g, '""')}"` : textVal;
      };
      rawOutputBuffer = [titleHeaders.map(formatCSVCell).join(","), ...contentRows.map(row => row.map(formatCSVCell).join(","))].join("\n");
      fileMimeType = "text/csv;charset=utf-8;";
      calculatedExtension = formatType === "excel" ? "xlsx" : "csv";
    } else if (formatType === "pdf") {
      let pdfLayoutText = `========================================================================\n                  DESAM DEVELOPERS - PERFORMANCE LEDGER AUDIT\n                  Report Compiled On: ${TODAY_STR} By: ${currentUser.name}\n========================================================================\n\n`;
      contentRows.forEach((row, i) => {
        pdfLayoutText += `${i + 1}. CLIENT PROFILE: ${row[1]}\n   • Phone: ${row[2]} | Email: ${row[3]}\n   • Scheme Context: ${row[4]} | Budget: ${row[5]}\n   • Source: ${row[6]} | Agent: ${row[7]}\n   • Status: [${row[8]}] | Logged: ${row[9]}\n\n`;
      });
    ...
    try {
      ...
      alert(`Export Error: ${exportError.message}`);
    }
  };
  ...
  const handleLoginSubmit = (e) => {
    ...
    if (account) { setCurrentUser(account); setLoginError(""); triggerToastAlert(`Welcome back, ${account.name}!`); }
    else setLoginError("Invalid profile matching credential set.");
  };
  ...
  const requestStatusTransitionPopup = (leadId, nextStatus) => {
    ...
    if (!target) return;
    setCustomPopup({ isOpen: true, leadId, targetValue: nextStatus, type: "status", title: "Confirm Status Shift", message: `Are you sure you want to transition "${target.name}" to the "${nextStatus}" track?` });
  };

  const requestOwnerAssignmentPopup = (leadId, personnelName) => {
    const target = leads.find(l => l.id === leadId);
    if (!target) return;
    setCustomPopup({ isOpen: true, leadId, targetValue: personnelName, type: "assign", title: "Confirm Lead Allocation", message: `Delegate tracking parameters for client "${target.name}" to team member "${personnelName}"?` });
  };

  const confirmCustomPopupAction = () => {
    const { leadId, targetValue, type } = customPopup;
    if (type === "status") {
      const histEntry = { date: TODAY_STR, by: currentUser.name, action: `Milestone status shifted manually to: ${targetValue}` };
      setLeads(leads.map(l => l.id === leadId ? { ...l, status: targetValue, history: [histEntry, ...l.history] } : l));
      if (selectedLead && selectedLead.id === leadId) setSelectedLead({ ...selectedLead, status: targetValue, history: [histEntry, ...selectedLead.history] });
      triggerToastAlert("Pipeline status successfully updated.");
    } else if (type === "assign") {
      const histEntry = { date: TODAY_STR, by: currentUser.name, action: `Routed lead to team desk: ${targetValue} (${currentUser.role})` };
      setLeads(leads.map(l => l.id === leadId ? { ...l, assignedTo: targetValue, assignedByRole: currentUser.role, status: targetValue === "Unassigned" ? "New" : "Assigned", history: [histEntry, ...l.history] } : l));
      ...
      lines.forEach(line => {
        ...
        if (columns.length >= 4) {
          ...
          newlyIngestedLeads.push({
            ...
            name: columns[0] || "Spreadsheet Lead", phone: parsedPhone, email: columns[2] || "",
            project: columns[3] || "Vishal Virinchi", location: columns[4] || "Inbound",
            budget: parseInt(columns[5]) || 65, source: columns[6] || "Website",
            assignedTo: "Unassigned", assignedByRole: "", status: "New", branch: branchHome,
            ...
            siteVisitTentativeDate: "", bookingUnit: "", bookingAmount: 0, bookingMode: "", bookingDate: "", regPending: false, regCompleted: false,
          });
        }
      });
  ...
  const handleCreateUserSubmit = (e) => {
    ...
    const createdUser = {
      id: Date.now(), name: newUserForm.name.trim(), email: `${cleanPrefix}@desam`,
      pass: newUserForm.pass, role: newUserForm.role,
      ...
      active: true, avatar: newUserForm.name.charAt(0).toUpperCase()
    };
  ...
  const handleDeleteUser = (userId) => {
    if (userId === currentUser.id) { triggerToastAlert("Cannot delete your own active session account."); return; }
  ...
  const handleUpdateUserSubmit = (e) => {
    ...
    const updatedUser = {
      ...editUserForm, name: editUserForm.name.trim(), email: `${cleanPrefix}@desam`,
  ...
  const commitManualFollowUpReport = (e) => {
    ...
    const updated = leads.map(l => {
      if (l.id === selectedLead.id) {
        const obj = { ...l, status: "Contacted", lastFollowUp: TODAY_STR, nextFollowUp: nextFollowUpDate, history: [{ date: TODAY_STR, by: currentUser.name, action: `[Follow-Up Entry]: ${followUpNotes.trim()} (Next review scheduled for: ${nextFollowUpDate})` }, ...l.history] };
        setSelectedLead(obj);
        return obj;
      }
      return l;
  ...
  const handleCreateLead = (e) => {
    ...
    if (databaseDoubleCheck) { setDuplicateConflictRecord(databaseDoubleCheck); return; }
    const created = {
      ...newLeadForm, id: Date.now(), phone: finalizedCleanPhone,
      ...
      history: [{ date: TODAY_STR, by: currentUser.name, action: "Lead Record Captured cleanly." }]
    };
  ...
  const handleCreateProject = (e) => {
    e.preventDefault();
    const createdProj = { ...newProjectForm, id: Date.now(), price: parseInt(newProjectForm.price) || 0, units: parseInt(newProjectForm.units) || 0, sold: parseInt(newProjectForm.sold) || 0 };
  ...
  const commitSiteWalkthroughLog = () => {
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, status: "Site Visit Completed", history: [{ date: svDate, by: currentUser.name, action: `[Walkthrough Validation]: Tour executed with family (${svFamily}). Feedback: ${svFeedback}` }, ...l.history] } : l));
    setSelectedLead(null);
    triggerToastAlert("Site walkthrough registered successfully.");
  };

  const commitFinancialBookingLog = () => {
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, status: "Booking Confirmed", bookingUnit: bkUnit, history: [{ date: TODAY_STR, by: currentUser.name, action: `[Advance Token Secured]: Allocated unit [${bkUnit}] with secured deposit.` }, ...l.history] } : l));
    setSelectedLead(null);
    ...
    <>
      <div>
        <div className="h-20 flex items-center px-5 border-b border-slate-800 bg-slate-950">
          <img src={DESAM_LOGO_ASSET} alt="Desam Developers Pvt Ltd" className="h-11 w-auto object-contain max-w-[220px]" />
        </div>
        <nav className="p-4 space-y-1">
          {[
            { id: "dashboard", icon: <Layers className="h-4 w-4" />, label: "VISUAL DASHBOARD" },
            { id: "leads", icon: <PhoneCall className="h-4 w-4" />, label: "LEAD CHANNELS" },
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type="text" required value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-9 pr-3 py-2.5 text-slate-200 focus:outline-none focus:border-orange-500" placeholder="Enter Username" />
                </div>
              </div>
              <div className="space-y-1.5">
    ...
    <div className="flex h-screen bg-slate-900 text-slate-100 font-sans antialiased overflow-hidden relative">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden bg-black/60 backdrop-blur-sm">
          <aside className="w-64 bg-slate-950 border-r border-slate-800 flex-col justify-between h-full flex">
            <SidebarContent />
          </aside>
            ...
            <div className="space-y-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-950 p-6 border border-slate-800 rounded-2xl">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Real-time active workspace dashboard engine.</p>
                </div>
              </div>
                ...
                <div className="bg-rose-950/40 border border-rose-500/30 p-4 lg:p-5 rounded-2xl flex items-start gap-4 shadow-xl">
                  ...
                  <div>
                    ...
                    <p className="text-rose-200/70 text-xs mt-1.5 leading-relaxed font-medium">
                      You have <span className="text-rose-400 font-bold">{unattendedManagerAlerts.length}</span> leads in "New" status unattended for over 48 hours.
                    </p>
                  </div>
                </div>
              )}
                    ...
                    {dashboardActionQueueLeads.map(l => (
                      <div key={l.id} className="bg-slate-900/60 border border-slate-800 p-4 rounded-xl flex flex-col justify-between space-y-3 relative overflow-hidden">
                        {l.assignedByRole === "Admin" && currentUser.role === "Manager" && (
                          <div className="absolute top-0 right-0 bg-rose-600 text-[8px] font-black tracking-wider uppercase px-2 py-0.5 rounded-bl text-white animate-pulse">⭐ Admin Priority</div>
                        )}
                        ...
                        <div>
                          ...
                          <div className="mt-2 flex gap-1.5 items-center">
                            <span className="text-[9px] px-2 py-0.5 font-bold uppercase rounded" style={{ backgroundColor: SC[l.status]?.bg || "rgba(255,255,255,0.05)", color: SC[l.status]?.text || "#FFF" }}>{l.status}</span>
                          </div>
                        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                  ...
                  <div className="h-[280px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartDataSource} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px', fontSize: '11px', color: '#f8fafc' }} cursor={{ fill: '#1e293b' }} />
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={chartDataStatus} cx="50%" cy="50%" innerRadius={70} outerRadius={100} paddingAngle={4} dataKey="value" stroke="none">
                          {chartDataStatus.map((entry, index) => <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}
                        </Pie>
                ...
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-xl col-span-1 lg:col-span-2 shadow-xl">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Executive Lead Allocation</h3>
                  <div className="h-[280px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={chartDataProject} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={4} dataKey="value" stroke="none">
                          {chartDataProject.map((entry, index) => <Cell key={`cell-${index}`} fill={PIE_COLORS[(index + 3) % PIE_COLORS.length]} />)}
                        </Pie>
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px', fontSize: '11px', color: '#f8fafc' }} />
                      </PieChart>
                    </ResponsiveContainer>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-slate-900 border-b border-slate-800 text-slate-400 font-bold uppercase tracking-wider">
                        ...
                        <th className="p-4 min-w-[160px]">Team Assignment</th>
                    ...
                    <tbody className="divide-y divide-slate-900 text-slate-300">
                      {processedLeads.length === 0 ? (
                        <tr><td colSpan={6} className="p-12 text-center text-slate-500 font-semibold italic">No active profiles inside tracking query bounds.</td></tr>
                      ) : (
                        processedLeads.map(l => (
                          <tr key={l.id} className="hover:bg-slate-900/30 transition-all">
                            ...
                            <td className="p-4">
                              <span className="bg-slate-900 border border-slate-800 text-slate-400 px-2 py-0.5 rounded text-[10px] font-bold font-mono">{l.source}</span>
                            </td>
                            <td className="p-4">
                              {["Admin", "Manager"].includes(currentUser.role) ? (
                                <select value={l.assignedTo} onChange={(e) => requestOwnerAssignmentPopup(l.id, e.target.value)} className="bg-slate-900 border border-slate-800 text-slate-200 rounded px-2.5 py-1.5 text-xs font-semibold focus:outline-none focus:border-orange-500 cursor-pointer max-w-[170px]">
                                  ...
                                  <optgroup label="Corporate Managers">
                                    {users.filter(u => u.role === "Manager").map(u => <option key={u.id} value={u.name}>{u.name} (Mgr)</option>)}
                                  </optgroup>
                                  <optgroup label="Regional Roster Staff">
                                    {users.filter(u => ["Executive", "Telecaller"].includes(u.role)).map(u => <option key={u.id} value={u.name}>{u.name} ({u.role})</option>)}
                                  </optgroup>
                                </select>
                              ) : (
                  ...
                  <div key={p.id} className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4 relative overflow-hidden shadow-xl">
                    ...
                    <div className="bg-slate-900/60 rounded-xl p-3 border border-slate-800 text-xs space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-slate-500 font-medium">Inventory Type:</span>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-2xl h-fit">
                  <div className="space-y-0.5">
                    <h3 className="text-sm font-black text-orange-400 flex items-center gap-2 uppercase tracking-wider"><UserPlus className="h-4 w-4" /> Deploy New Corporate Profile</h3>
                    <p className="text-xs text-slate-400">Build access credentials and assign tracking branch parameters.</p>
                  </div>
                  <form onSubmit={handleCreateUserSubmit} className="space-y-4 text-xs pt-1">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        <label className="text-slate-400 font-bold">Branch Boundary</label>
                        <select value={newUserForm.branch} disabled={newUserForm.role === "Admin"} onChange={(e)=>setNewUserForm({...newUserForm, branch: e.target.value})} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-300 focus:outline-none disabled:opacity-40">
                    ...
                    <button type="submit" className="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-2.5 rounded-xl uppercase tracking-wider transition-all shadow-md mt-1">Compile Profile Authorization</button>
                  </form>
                </div>

                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-2xl h-fit">
                  <div className="space-y-0.5">
                    <h3 className="text-sm font-black text-orange-400 flex items-center gap-2 uppercase tracking-wider"><Upload className="h-4 w-4" /> SpreadSheet Excel/CSV Ingestion Engine</h3>
                    <p className="text-xs text-slate-400">Bulk map customer data streams. Paste matching rows directly using tabular block separations.</p>
                  </div>
                  <form onSubmit={handleDataImportSubmit} className="space-y-4 pt-1">
                    <textarea rows={5} value={importText} onChange={(e)=>setImportText(e.target.value)} placeholder={"Suresh Kumar\t9840011234\tsuresh@gmail.com\tVishal Virinchi\tMadurai\t65"} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-slate-200 focus:outline-none focus:border-orange-500 font-mono leading-relaxed" />
                    <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-3 flex gap-3 items-start text-xs text-slate-400">
                      ...
                      <p className="leading-relaxed">Verify the Project Name column maps accurately into your static project catalogs.</p>
                    </div>
            ...
            <div className="space-y-8 pb-20">
                  ...
                  {projectwiseAnalysis.map(([project, stats]) => (
                    <div key={project} className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <span className="text-[10px] font-mono text-emerald-500 font-bold">{stats.total > 0 ? Math.round((stats.converted / stats.total) * 100) : 0}% Conversion</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 lg:p-6 shadow-xl w-full">
                <div className="space-y-0.5 mb-4">
                  <h3 className="text-sm font-black text-orange-400 flex items-center gap-2 uppercase tracking-wider"><BarChart3 className="h-4 w-4" /> Executive Pipeline Summary Report</h3>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="text-slate-500 font-bold border-b border-slate-900 uppercase">
                        ...
                        <th className="pb-2 text-center text-slate-300">Untouched</th>
                    ...
                    <tbody className="divide-y divide-slate-900 text-slate-300">
                      ...
                      {executiveSummaryData.length === 0 && (
                        <tr><td colSpan={7} className="py-8 text-center text-slate-500 italic">No data available for the selected parameters.</td></tr>
                      )}
                    </tbody>
                    {executiveSummaryData.length > 0 && (
              ...
              {/* Full Lead Ledger Table */}
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 lg:p-6 shadow-xl w-full">
                <h3 className="text-sm font-black text-slate-400 mb-4 uppercase tracking-widest">Full Lead Ledger</h3>
                <div className="overflow-x-auto w-full">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="text-slate-500 font-bold border-b border-slate-900 uppercase">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 w-full max-w-md rounded-2xl p-6 space-y-4 shadow-2xl text-center">
            ...
            <div className="grid grid-cols-2 gap-2.5 pt-2 text-xs uppercase font-black tracking-wider">
              <button onClick={() => setCustomPopup({ isOpen: false, leadId: null, targetValue: "", type: "status", title: "", message: "" })} className="w-full bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-white py-2.5 rounded-xl transition-all">✕ Discard</button>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl relative">
            ...
            <form onSubmit={handleUpdateUserSubmit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-slate-400 font-bold">Branch Boundary</label>
                  <select value={editUserForm.branch} disabled={editUserForm.role === "Admin"} onChange={(e)=>setEditUserForm({...editUserForm, branch: e.target.value})} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-300 focus:outline-none disabled:opacity-40">
                    {BRANCHES.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
              </div>
              ...
              <button type="submit" className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 text-white font-black py-3 rounded-xl uppercase tracking-wider transition-all shadow-md mt-2">Update Profile Authorization</button>
            </form>
          </div>
        </div>
      )}
          ...
          <div className="bg-slate-950 border border-slate-800 w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-2xl p-6 space-y-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-b border-slate-900 pb-3 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-slate-500 tracking-wide font-mono">Reference ID: #{selectedLead.id} • Assigned: <span className="text-slate-300 font-bold">{selectedLead.assignedTo}</span></p>
                {selectedLead.siteVisitTentativeDate && (
                  <p className="text-xs font-black text-purple-400 mt-1 flex items-center gap-1.5 bg-purple-500/10 border border-purple-500/20 px-3 py-1 rounded-xl w-fit">
                    <Calendar className="h-3.5 w-3.5" /> WALKTHROUGH SCHEDULED: {selectedLead.siteVisitTentativeDate}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-4 flex-wrap sm:flex-nowrap">
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase tracking-wider text-slate-500 block">Milestone Status</label>
                ...
                {["Admin", "Manager"].includes(currentUser.role) && (
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase tracking-wider text-slate-500 block">Staff Assignment</label>
            ...
            {selectedLead.status === "Site Visit Planned" && (
              <form onSubmit={commitTentativeWalkthroughPlan} className="bg-purple-950/40 p-4 border border-purple-500/30 rounded-xl space-y-3 text-xs">
                <div className="flex items-center gap-2 text-purple-400 font-black tracking-wide uppercase"><Calendar className="h-4 w-4" /> Log Tentative Site Visit Date</div>
                <div className="space-y-1">
            ...
            <div className="bg-slate-900/80 p-4 border border-slate-800 rounded-xl space-y-2 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-200">
                ...
                <div className="flex items-center gap-2 bg-slate-950 px-3 py-2 rounded-lg border border-slate-800 col-span-1 sm:col-span-2">
                  <span className="text-slate-500 font-mono font-bold text-[9px] uppercase">Email:</span>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-5">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2"><Clock className="h-4 w-4 text-orange-500" /> INTERACTION TIMELINE JOURNAL</h4>
              ...
              <div className="relative max-h-[280px] overflow-y-auto pr-2 space-y-0 pt-2 pl-2">
                ...
                {selectedLead.history && selectedLead.history.length > 0 ? selectedLead.history.map((log, index) => {
                  ...
                  <p className="text-xs text-slate-500 italic text-center py-6">No workflow trail tracked yet.</p>
                )}
              </div>
            </div>
              ...
              <div className="space-y-5 text-xs">
                ...
                <form onSubmit={commitManualFollowUpReport} className="bg-slate-900/50 p-4 border border-slate-800 rounded-xl space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea rows={2} required value={followUpNotes} onChange={(e)=>setFollowUpNotes(e.target.value)} placeholder="Type conversation outcomes..." className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2 text-slate-200 focus:outline-none" />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="bg-slate-900/50 p-4 border border-slate-800 rounded-xl space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="space-y-1"><label className="text-slate-400 text-[10px]">Attended Family Co-Buyers</label><input type="text" value={svFamily} onChange={(e)=>setSvFamily(e.target.value)} placeholder="Spouse, etc." className="w-full bg-slate-950 border border-slate-800 rounded p-1.5 text-slate-200" /></div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-slate-400 text-[10px]">Site Walkthrough Feedback Notes *</label>
                    <textarea rows={1} value={svFeedback} onChange={(e)=>setSvFeedback(e.target.value)} placeholder="Enter feedback..." className="w-full bg-slate-950 border border-slate-800 rounded p-2 text-slate-200 focus:outline-none" />
                  </div>
            ...
            {duplicateConflictRecord && (
              <div className="absolute inset-x-6 top-16 bottom-6 bg-slate-950/95 border border-rose-500/40 rounded-xl p-5 z-20 flex flex-col justify-between space-y-3 backdrop-blur-md">
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-rose-400 font-black tracking-wide text-xs"><AlertTriangle className="h-5 w-5 animate-bounce" /> ⚠️ DUPLICATE PHONE NUMBER DETECTED</div>
                  <p className="text-slate-400 text-[11px] leading-relaxed">The contact number <span className="text-white font-black font-mono bg-slate-900 px-1.5 py-0.5 rounded">{duplicateConflictRecord.phone}</span> is already logged.</p>
                  <div className="bg-rose-500/10 border border-rose-500/20 rounded-lg p-3 space-y-1.5 text-xs">
                    <div className="flex justify-between items-center"><span className="text-rose-300 font-semibold">Existing Customer:</span><span className="text-white font-bold">{duplicateConflictRecord.name}</span></div>
                    <div className="flex justify-between items-center"><span className="text-rose-300 font-semibold">Assigned Handler:</span><span className="text-white font-bold">{duplicateConflictRecord.assignedTo}</span></div>
                  </div>
                </div>
            ...
            <form onSubmit={handleCreateLead} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input type="text" required value={newLeadForm.name} onChange={(e)=>setNewLeadForm({...newLeadForm, name: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 focus:outline-none focus:border-orange-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Attribution Channel Source</label>
                  <select value={newLeadForm.source} onChange={(e)=>setNewLeadForm({...newLeadForm, source: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">
                    {sources.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input type="text" required value={newLeadForm.phone} onChange={(e) => handlePhoneInputChange(e.target.value, false)} placeholder="e.g. 9840011234" className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 font-mono font-bold focus:outline-none focus:border-orange-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Alternate Contact Number</label>
                  <input type="text" value={newLeadForm.altPhone} onChange={(e) => handlePhoneInputChange(e.target.value, true)} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 font-mono focus:outline-none focus:border-orange-500" />
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-slate-400 font-semibold">Target Project</label>
                <select value={newLeadForm.project} onChange={(e)=>setNewLeadForm({...newLeadForm, project: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">
                  {projects.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
                </select>
              </div>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleCreateProject} className="space-y-4 text-xs">
              <div className="space-y-1">
                ...
                <input type="text" required value={newProjectForm.name} onChange={(e)=>setNewProjectForm({...newProjectForm, name: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 focus:outline-none focus:border-orange-500" placeholder="e.g. Vishal Virinchi Phase II" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Location Parameter *</label>
                  <input type="text" required value={newProjectForm.location} onChange={(e)=>setNewProjectForm({...newProjectForm, location: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 focus:outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Team Desk Branch</label>
                  <select value={newProjectForm.branch} onChange={(e)=>setNewProjectForm({...newProjectForm, branch: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">
                    {BRANCHES.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  ...
                  <select value={newProjectForm.type} onChange={(e)=>setNewProjectForm({...newProjectForm, type: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">
                    {PROJECT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Base Price (₹L)</label>
                  <input type="number" value={newProjectForm.price} onChange={(e)=>setNewProjectForm({...newProjectForm, price: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 font-mono" />
                </div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Lifecycle Track</label>
                  <select value={newProjectForm.status} onChange={(e)=>setNewProjectForm({...newProjectForm, status: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">
                    {PROJECT_STATUSES.map(st => <option key={st} value={st}>{st}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input type="number" value={newProjectForm.units} onChange={(e)=>setNewProjectForm({...newProjectForm, units: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200" />
                </div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-semibold">Initial Sold Units</label>
                  <input type="number" value={newProjectForm.sold} onChange={(e)=>setNewProjectForm({...newProjectForm, sold: e.target.value})} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200" />
                </div>
              </div>
