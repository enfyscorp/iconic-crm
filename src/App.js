...
export default function App() {
  ...
  const [newActivityForm, setNewActivityForm] = useState({
    date: TODAY_STR, executive: "", project: "Desam Garden", source: "Own Leads",
    callsMade: 0, callStatus: "Warm", followup: 0, siteVisit: 0, booking: 0,
    registration: 0, cancellation: 0, collection: 0, remark: ""
  });
  ...
  const processedLeads = useMemo(() => {
    ...
    if (globalSearch.trim()) {
      ...
      result = result.filter(l => l.name.toLowerCase().includes(term) || l.phone.includes(term) || l.project.toLowerCase().includes(term) || l.status.toLowerCase().includes(term));
    }
  ...
  const filteredActivityLogs = useMemo(() => {
    ...
    if (currentUser && currentUser.role !== "Admin" && currentUser.role !== "Manager") {
      logs = logs.filter(l => l.executive === currentUser.name);
    }
  ...
  const callsTrendData = useMemo(() => {
    ...
    filteredActivityLogs.forEach(l => {
      if (!map[l.date]) map[l.date] = { date: l.date, calls: 0, followups: 0, siteVisits: 0, bookings: 0 };
      map[l.date].calls += l.callsMade || 0;
      map[l.date].followups += l.followup || 0;
      map[l.date].siteVisits += l.siteVisit || 0;
      map[l.date].bookings += l.booking || 0;
    });
  ...
  const projectPerfData = useMemo(() => {
    const map = {};
    filteredActivityLogs.forEach(l => {
      if (!map[l.project]) map[l.project] = { project: l.project, calls: 0, followups: 0, siteVisits: 0, bookings: 0 };
      map[l.project].calls += l.callsMade || 0;
      map[l.project].followups += l.followup || 0;
      map[l.project].siteVisits += l.siteVisit || 0;
      map[l.project].bookings += l.booking || 0;
    });
  ...
  const sourcewisePieData = useMemo(() => {
    const map = {};
    filteredActivityLogs.forEach(l => {
      if (!map[l.source]) map[l.source] = 0;
      map[l.source] += l.callsMade || 0;
    });
  ...
  const execPerfChartData = useMemo(() => {
    const map = {};
    filteredActivityLogs.forEach(l => {
      if (!map[l.executive]) map[l.executive] = { name: l.executive, calls: 0, followups: 0, siteVisits: 0, bookings: 0 };
      map[l.executive].calls += l.callsMade || 0;
      map[l.executive].followups += l.followup || 0;
      map[l.executive].siteVisits += l.siteVisit || 0;
      map[l.executive].bookings += l.booking || 0;
    });
    return Object.values(map).sort((a, b) => b.calls - a.calls);
  }, [filteredActivityLogs]);
  ...
  const execDetailedMatrix = useMemo(() => {
    const map = {};
    filteredActivityLogs.forEach(l => {
      if (!map[l.executive]) map[l.executive] = {
        name: l.executive, calls: 0, followups: 0, siteVisits: 0,
        bookings: 0, registrations: 0, cancellations: 0, collection: 0
      };
      map[l.executive].calls += l.callsMade || 0;
      map[l.executive].followups += l.followup || 0;
      map[l.executive].siteVisits += l.siteVisit || 0;
      map[l.executive].bookings += l.booking || 0;
      map[l.executive].registrations += l.registration || 0;
      map[l.executive].cancellations += l.cancellation || 0;
      map[l.executive].collection += l.collection || 0;
    });
    return Object.values(map).sort((a, b) => b.calls - a.calls);
  }, [filteredActivityLogs]);
  ...
  const reportSummaryMatrix = useMemo(() => {
    ...
    const data = {};
    metrics.forEach(m => { data[m] = {}; projectNames.forEach(p => { data[m][p] = 0; }); });
    filteredActivityLogs.forEach(l => {
      if (data["Calls made"]) data["Calls made"][l.project] = (data["Calls made"][l.project] || 0) + (l.callsMade || 0);
      if (data["Followup"]) data["Followup"][l.project] = (data["Followup"][l.project] || 0) + (l.followup || 0);
      if (data["Site Visit"]) data["Site Visit"][l.project] = (data["Site Visit"][l.project] || 0) + (l.siteVisit || 0);
      if (data["Booking"]) data["Booking"][l.project] = (data["Booking"][l.project] || 0) + (l.booking || 0);
      if (data["Registration"]) data["Registration"][l.project] = (data["Registration"][l.project] || 0) + (l.registration || 0);
      if (data["Cancellation"]) data["Cancellation"][l.project] = (data["Cancellation"][l.project] || 0) + (l.cancellation || 0);
    });
  ...
  const sourcewiseMatrix = useMemo(() => {
    ...
    const data = {};
    srcNames.forEach(s => { data[s] = {}; execNames.forEach(e => { data[s][e] = 0; }); });
    let srcTotals = {};
    filteredActivityLogs.forEach(l => {
      if (data[l.source]) data[l.source][l.executive] = (data[l.source][l.executive] || 0) + (l.callsMade || 0);
      srcTotals[l.source] = (srcTotals[l.source] || 0) + (l.callsMade || 0);
    });
  ...
  const projectwiseMatrix = useMemo(() => {
    ...
    const data = {};
    projNames.forEach(p => { data[p] = {}; execNames.forEach(e => { data[p][e] = 0; }); });
    let projTotals = {};
    filteredActivityLogs.forEach(l => {
      if (data[l.project]) data[l.project][l.executive] = (data[l.project][l.executive] || 0) + (l.callsMade || 0);
      projTotals[l.project] = (projTotals[l.project] || 0) + (l.callsMade || 0);
    });
  ...
  const unattendedManagerAlerts = useMemo(() => {
    ...
    return leads.filter(l => {
      if (l.branch !== currentUser.branch || l.status !== "New") return false;
      return (new Date(TODAY_STR) - new Date(l.dateCreated)) / (1000 * 60 * 60 * 24) >= 2;
    });
  }, [leads, currentUser]);
  ...
  const chartDataExecutive = useMemo(() => {
    const counts = {};
    processedLeads.forEach(l => { if (l.assignedTo && l.assignedTo !== "Unassigned") counts[l.assignedTo] = (counts[l.assignedTo] || 0) + 1; });
    return Object.keys(counts).map(key => ({ name: key, count: counts[key] })).sort((a, b) => b.count - a.count);
  }, [processedLeads]);
  ...
  const executiveSummaryData = useMemo(() => {
    ...
    visibleUsers.forEach(u => {
      if (["Executive", "Telecaller"].includes(u.role)) execMap[u.name] = { name: u.name, total: 0, new: 0, active: 0, siteVisits: 0, bookings: 0, dead: 0 };
    });
    ...
    processedLeads.forEach(l => {
      ...
      if (!execMap[exec]) execMap[exec] = { name: exec, total: 0, new: 0, active: 0, siteVisits: 0, bookings: 0, dead: 0 };
      execMap[exec].total += 1;
      if (l.status === "New") execMap[exec].new += 1;
      else if (["Assigned", "Contacted", "Follow-Up", "Negotiation"].includes(l.status)) execMap[exec].active += 1;
      else if (["Site Visit Planned", "Site Visit Completed"].includes(l.status)) execMap[exec].siteVisits += 1;
      else if (["Booking Pending", "Booking Confirmed", "Closed"].includes(l.status)) execMap[exec].bookings += 1;
      else if (["Not Interested", "RNR", "Switched Off", "Wrong Number"].includes(l.status)) execMap[exec].dead += 1;
    });
    return Object.values(execMap).filter(e => e.total > 0 || visibleUsers.some(u => u.name === e.name)).sort((a, b) => b.total - a.total);
  }, [processedLeads, visibleUsers]);

  const sourcewiseAnalysis = useMemo(() => {
    const data = {};
    processedLeads.forEach(l => { if (!data[l.source]) data[l.source] = { total: 0 }; data[l.source].total += 1; });
  ...
  const handlePhoneInputChange = (val, isAlt = false) => {
    ...
    if (clean.length >= 10) {
      const dup = leads.find(l => stripPhone(l.phone) === clean);
      if (dup) { setDuplicateConflictRecord(dup); return; }
    }
  ...
  const handleInlineMilestoneStatusChange = (leadId, targetStatus) => {
    ...
    setLeads(leads.map(l => l.id === leadId ? { ...l, status: targetStatus, history: [log, ...l.history] } : l));
  ...
  const commitTentativeWalkthroughPlan = (e) => {
    ...
    const log = { date: TODAY_STR, by: currentUser.name, action: `[SITE VISIT PLANNED]: Date set to ${tentativeWalkthroughDateInput}.` };
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, status: "Site Visit Planned", siteVisitTentativeDate: tentativeWalkthroughDateInput, history: [log, ...l.history] } : l));
    setSelectedLead(prev => ({ ...prev, status: "Site Visit Planned", siteVisitTentativeDate: tentativeWalkthroughDateInput, history: [log, ...prev.history] }));
    setTentativeWalkthroughDateInput("");
    triggerToastAlert("Tentative date saved.");
  };
  ...
  const handleLoginSubmit = (e) => {
    e.preventDefault();
    const acc = users.find(u => u.email.toLowerCase() === loginEmail.toLowerCase().trim() && u.pass === loginPassword && u.active);
    if (acc) { setCurrentUser(acc); setLoginError(""); triggerToastAlert(`Welcome back, ${acc.name}!`); }
    else setLoginError("Invalid credentials.");
  };

  const handleLogout = () => {
    setCurrentUser(null); setLoginEmail(""); setLoginPassword(""); setGlobalSearch("");
    setActiveTab("dashboard"); setNavHistory([]); setIsMobileMenuOpen(false);
  };

  const requestStatusTransitionPopup = (leadId, nextStatus) => {
    const target = leads.find(l => l.id === leadId);
    if (!target) return;
    setCustomPopup({ isOpen: true, leadId, targetValue: nextStatus, type: "status", title: "Confirm Status Shift", message: `Transition "${target.name}" to "${nextStatus}"?` });
  };

  const requestOwnerAssignmentPopup = (leadId, personnelName) => {
    const target = leads.find(l => l.id === leadId);
    if (!target) return;
    setCustomPopup({ isOpen: true, leadId, targetValue: personnelName, type: "assign", title: "Confirm Assignment", message: `Assign "${target.name}" to "${personnelName}"?` });
  };

  const confirmCustomPopupAction = () => {
    const { leadId, targetValue, type } = customPopup;
    if (type === "status") {
      const log = { date: TODAY_STR, by: currentUser.name, action: `Status updated to: ${targetValue}` };
      setLeads(leads.map(l => l.id === leadId ? { ...l, status: targetValue, history: [log, ...l.history] } : l));
      if (selectedLead && selectedLead.id === leadId) setSelectedLead({ ...selectedLead, status: targetValue, history: [log, ...selectedLead.history] });
      triggerToastAlert("Status updated.");
    } else if (type === "assign") {
      const log = { date: TODAY_STR, by: currentUser.name, action: `Assigned to: ${targetValue}` };
      setLeads(leads.map(l => l.id === leadId ? { ...l, assignedTo: targetValue, assignedByRole: currentUser.role, status: targetValue === "Unassigned" ? "New" : "Assigned", history: [log, ...l.history] } : l));
      setSelectedLead(null);
      triggerToastAlert(`Assigned to ${targetValue}`);
    }
  ...
  const handleDataImportSubmit = (e) => {
    ...
    try {
      ...
      lines.forEach(line => {
        ...
        if (cols.length >= 4) {
          const matchedProj = projects.find(p => p.name.toLowerCase() === (cols[3] || "").toLowerCase().trim());
          const branchHome = matchedProj ? matchedProj.branch : "Madurai Desk";
          newLeads.push({ id: Date.now() + Math.floor(Math.random() * 10000), name: cols[0] || "Lead", phone: stripPhone(cols[1] || "00000"), email: cols[2] || "", project: cols[3] || "Vishal Virinchi", location: cols[4] || "Inbound", budget: parseInt(cols[5]) || 65, source: cols[6] || "Website", assignedTo: "Unassigned", assignedByRole: "", status: "New", branch: branchHome, dateCreated: TODAY_STR, lastFollowUp: "None", nextFollowUp: TODAY_STR, history: [{ date: TODAY_STR, by: currentUser.name, action: "Imported via Excel paste." }], siteVisitTentativeDate: "", bookingUnit: "", bookingAmount: 0, bookingMode: "", bookingDate: "", regPending: false, regCompleted: false });
        }
      });
      if (newLeads.length > 0) { setLeads([...newLeads, ...leads]); triggerToastAlert(`Imported ${newLeads.length} leads.`); setImportText(""); }
  ...
  const handleCreateUserSubmit = (e) => {
    ...
    const u = { id: Date.now(), name: newUserForm.name.trim(), email: `${prefix}@desam`, pass: newUserForm.pass, role: newUserForm.role, branch: newUserForm.role === "Admin" ? "All Branches" : newUserForm.branch, phone: stripPhone(newUserForm.phone) || "9840000000", active: true, avatar: newUserForm.name.charAt(0).toUpperCase() };
    setUsers([...users, u]);
    setNewUserForm({ name: "", emailPrefix: "", pass: "", role: "Executive", branch: "Madurai Desk", phone: "" });
    triggerToastAlert(`Profile for ${u.name} created.`);
  };
  ...
  const handleUpdateUserSubmit = (e) => {
    ...
    const u = { ...editUserForm, name: editUserForm.name.trim(), email: `${prefix}@desam`, branch: editUserForm.role === "Admin" ? "All Branches" : editUserForm.branch, phone: stripPhone(editUserForm.phone) || "9840000000", avatar: editUserForm.name.charAt(0).toUpperCase() };
    setUsers(users.map(x => x.id === u.id ? u : x));
    setIsEditUserModalOpen(false); setEditUserForm(null);
  ...
  const commitManualFollowUpReport = (e) => {
    ...
    const updated = leads.map(l => {
      if (l.id === selectedLead.id) {
        const obj = { ...l, status: "Contacted", lastFollowUp: TODAY_STR, nextFollowUp: nextFollowUpDate, history: [{ date: TODAY_STR, by: currentUser.name, action: `[Follow-Up]: ${followUpNotes.trim()} (Next: ${nextFollowUpDate})` }, ...l.history] };
        setSelectedLead(obj); return obj;
      }
      return l;
    });
  ...
  const handleCreateLead = (e) => {
    e.preventDefault();
    const phone = stripPhone(newLeadForm.phone);
    const dup = leads.find(l => stripPhone(l.phone) === phone);
    if (dup) { setDuplicateConflictRecord(dup); return; }
    const created = { ...newLeadForm, id: Date.now(), phone, altPhone: stripPhone(newLeadForm.altPhone), branch: currentUser.role === "Admin" ? "Madurai Desk" : currentUser.branch, dateCreated: TODAY_STR, lastFollowUp: "None", nextFollowUp: TODAY_STR, assignedByRole: "", bookingUnit: "", bookingAmount: 0, bookingMode: "", bookingDate: "", regPending: false, regCompleted: false, siteVisitTentativeDate: "", history: [{ date: TODAY_STR, by: currentUser.name, action: "Lead captured." }] };
    ...
    triggerToastAlert("Lead created.");
  };

  const handleCreateProject = (e) => {
    e.preventDefault();
    const p = { ...newProjectForm, id: Date.now(), price: parseInt(newProjectForm.price) || 0, units: parseInt(newProjectForm.units) || 0, sold: parseInt(newProjectForm.sold) || 0 };
    setProjects([p, ...projects]);
    setIsProjectModalOpen(false);
    setNewProjectForm({ name: "", location: "", branch: "Madurai Desk", type: "Plot", price: 30, units: 50, sold: 0, status: "Pre-Launch" });
    triggerToastAlert(`Project "${p.name}" added.`);
  };

  const handleCreateActivityLog = (e) => {
    e.preventDefault();
    const log = { ...newActivityForm, id: Date.now(), callsMade: parseInt(newActivityForm.callsMade) || 0, followup: parseInt(newActivityForm.followup) || 0, siteVisit: parseInt(newActivityForm.siteVisit) || 0, booking: parseInt(newActivityForm.booking) || 0, registration: parseInt(newActivityForm.registration) || 0, cancellation: parseInt(newActivityForm.cancellation) || 0, collection: parseInt(newActivityForm.collection) || 0 };
    setActivityLogs([log, ...activityLogs]);
    setIsActivityLogModalOpen(false);
    setNewActivityForm({ date: TODAY_STR, executive: "", project: "Desam Garden", source: "Own Leads", callsMade: 0, callStatus: "Warm", followup: 0, siteVisit: 0, booking: 0, registration: 0, cancellation: 0, collection: 0, remark: "" });
    triggerToastAlert("Activity log saved.");
  };

  const commitSiteWalkthroughLog = () => {
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, status: "Site Visit Completed", history: [{ date: svDate, by: currentUser.name, action: `[Site Visit]: Family: ${svFamily}. Feedback: ${svFeedback}` }, ...l.history] } : l));
    setSelectedLead(null); triggerToastAlert("Site visit logged.");
  };

  const commitFinancialBookingLog = () => {
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, status: "Booking Confirmed", bookingUnit: bkUnit, history: [{ date: TODAY_STR, by: currentUser.name, action: `[Booking]: Unit [${bkUnit}] booked.` }, ...l.history] } : l));
    ...
    <>
      <div>
        <div className="h-20 flex items-center px-5 border-b border-slate-800 bg-slate-950">
          <img src={DESAM_LOGO_ASSET} alt="Desam Developers" className="h-11 w-auto object-contain max-w-[220px]" />
        </div>
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans antialiased text-slate-200">
        <div className="sm:mx-auto w-full max-w-md text-center space-y-4">
          <div className="flex justify-center mb-2">
            <img src={DESAM_LOGO_ASSET} alt="Desam Developers Logo" className="h-16 w-auto object-contain drop-shadow-lg" />
          </div>
          ...
          <div className="bg-slate-950 py-8 px-6 border border-slate-800 rounded-2xl shadow-2xl space-y-6">
            <form onSubmit={handleLoginSubmit} className="space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <div className="relative"><Mail className="absolute left-3 top-2.5 h-4 w-4 text-slate-500" /><input type="text" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-9 pr-3 py-2.5 text-slate-200 focus:outline-none focus:border-orange-500" placeholder="Enter Username" /></div>
              </div>
              <div className="space-y-1.5">
                <label className="text-slate-400 font-bold uppercase tracking-wide">Password</label>
                <div className="relative"><Lock className="absolute left-3 top-2.5 h-4 w-4 text-slate-500" /><input type="password" required value={loginPassword} onChange={e => setLoginPassword(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-9 pr-3 py-2.5 text-slate-200 focus:outline-none focus:border-orange-500" placeholder="••••••••" /></div>
              </div>
    ...
    <div className="flex h-screen bg-slate-900 text-slate-100 font-sans antialiased overflow-hidden relative">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden bg-black/60 backdrop-blur-sm">
          <aside className="w-64 bg-slate-950 border-r border-slate-800 flex-col justify-between h-full flex"><SidebarContent /></aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        ...
        <header className="h-16 bg-slate-950 border-b border-slate-800 flex items-center justify-between px-4 lg:px-8 z-10 gap-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            {navHistory.length > 0 && (
              <button onClick={navigateBack} className="flex items-center gap-1.5 p-2 bg-slate-900 hover:bg-slate-800 rounded-xl border border-slate-800 text-slate-400 hover:text-white transition-colors text-xs font-bold uppercase tracking-wide" title="Go back">
                <ArrowLeft className="h-4 w-4" /><span className="hidden sm:inline">Back</span>
              </button>
            )}
            <div className="relative w-48 sm:w-80 hidden sm:block">
              <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-500" />
              <input type="text" value={globalSearch} onChange={e => setGlobalSearch(e.target.value)} placeholder="Search leads..." className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-9 pr-4 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-orange-500" />
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-8 w-full">
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-950 p-6 border border-slate-800 rounded-2xl">
                <div>
                  <h1 className="text-xl lg:text-2xl font-black text-white tracking-tight">Sales Performance Dashboard</h1>
                  <p className="text-xs text-slate-400 mt-0.5">Real-time activity & lead pipeline overview.</p>
                </div>
              </div>
              ...
              {unattendedManagerAlerts.length > 0 && (
                <div className="bg-rose-950/40 border border-rose-500/30 p-4 lg:p-5 rounded-2xl flex items-start gap-4 shadow-xl">
                  <div className="bg-rose-500/20 p-2 rounded-full border border-rose-500/30 mt-0.5"><AlertTriangle className="h-5 w-5 text-rose-500 animate-pulse" /></div>
                  <div>
                    <h3 className="text-rose-400 font-black text-xs uppercase tracking-wider">Unattended Leads Alert</h3>
                    <p className="text-rose-200/70 text-xs mt-1.5 font-medium">{unattendedManagerAlerts.length} leads in "New" status unattended for 48+ hours.</p>
                  </div>
                </div>
              )}
                      ...
                      <div key={l.id} className="bg-slate-900/60 border border-slate-800 p-4 rounded-xl flex flex-col justify-between space-y-3 relative overflow-hidden">
                        ...
                        <div>
                          <div className="flex justify-between items-start pr-16">
                            ...
                            <span className="text-[9px] bg-slate-950 border border-slate-800 text-slate-400 px-2 py-0.5 rounded font-mono font-bold">{l.source}</span>
                          </div>
                        ...
                        <button onClick={() => setSelectedLead(l)} className="w-full bg-orange-600/10 hover:bg-orange-600 border border-orange-500/20 text-[10px] text-orange-400 hover:text-white font-black py-1.5 rounded-lg tracking-wide uppercase transition-all">
                          {["Manager", "Admin"].includes(currentUser.role) ? "⚡ Delegate" : "📝 Open Workspace"}
                        </button>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-xl col-span-1 lg:col-span-2 shadow-xl">
                  ...
                  <div className="h-[260px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={callsTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                        <XAxis dataKey="date" tick={{ fontSize: 9, fill: '#64748b' }} axisLine={false} tickLine={false} angle={-35} textAnchor="end" />
                        <YAxis tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px', fontSize: '11px', color: '#f8fafc' }} />
                        <Legend wrapperStyle={{ fontSize: '10px' }} />
                        <Line type="monotone" dataKey="calls" stroke="#ea580c" strokeWidth={2.5} dot={{ r: 3 }} name="Calls" />
                        <Line type="monotone" dataKey="followups" stroke="#3b82f6" strokeWidth={2} dot={{ r: 2 }} name="Followups" />
                        <Line type="monotone" dataKey="siteVisits" stroke="#10b981" strokeWidth={2} dot={{ r: 2 }} name="Site Visits" />
                        <Line type="monotone" dataKey="bookings" stroke="#8b5cf6" strokeWidth={2} dot={{ r: 2 }} name="Bookings" />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-xl shadow-xl flex flex-col">
                  ...
                  <div className="h-[200px] w-full relative">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={sourcewisePieData} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value" stroke="none">
                          {sourcewisePieData.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                        </Pie>
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px', fontSize: '11px', color: '#f8fafc' }} />
                      </PieChart>
                    </ResponsiveContainer>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {sourcewisePieData.slice(0, 6).map((s, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-xl shadow-xl">
                  ...
                  <div className="h-[260px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={projectPerfData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                        <XAxis dataKey="project" tick={{ fontSize: 9, fill: '#64748b' }} axisLine={false} tickLine={false} angle={-30} textAnchor="end" />
                        <YAxis tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px', fontSize: '11px', color: '#f8fafc' }} />
                        <Bar dataKey="calls" fill="#ea580c" radius={[3, 3, 0, 0]} barSize={20} name="Calls" />
                        <Bar dataKey="followups" fill="#3b82f6" radius={[3, 3, 0, 0]} barSize={20} name="Followups" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-xl shadow-xl">
                  ...
                  <div className="h-[260px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={execPerfChartData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                        <XAxis dataKey="name" tick={{ fontSize: 9, fill: '#64748b' }} axisLine={false} tickLine={false} angle={-30} textAnchor="end" />
                        <YAxis tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px', fontSize: '11px', color: '#f8fafc' }} />
                        <Bar dataKey="calls" fill="#ea580c" radius={[3, 3, 0, 0]} barSize={18} name="Calls" />
                        <Bar dataKey="siteVisits" fill="#10b981" radius={[3, 3, 0, 0]} barSize={18} name="Site Visits" />
                        <Bar dataKey="bookings" fill="#8b5cf6" radius={[3, 3, 0, 0]} barSize={18} name="Bookings" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="border-b border-slate-800 text-slate-500 uppercase text-[10px]">
                        ...
                        <th className="pb-2 text-center text-orange-400">Call Made</th>
                        <th className="pb-2 text-center text-blue-400">Follow ups</th>
                        <th className="pb-2 text-center text-emerald-400">Site Visits</th>
                        <th className="pb-2 text-center text-purple-400">Bookings</th>
                        <th className="pb-2 text-center text-amber-400">Regi.</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-900">
                      {execDetailedMatrix.map((e, i) => (
                        <tr key={i} className="hover:bg-slate-900/20">
                          <td className="py-2.5 pl-2 font-bold text-white">{e.name}</td>
                          <td className="py-2.5 text-center font-mono font-bold text-orange-400">{e.calls.toLocaleString()}</td>
                          <td className="py-2.5 text-center font-mono text-blue-400">{e.followups}</td>
                          <td className="py-2.5 text-center font-mono text-emerald-400">{e.siteVisits}</td>
                          <td className="py-2.5 text-center font-mono text-purple-400">{e.bookings}</td>
                          <td className="py-2.5 text-center font-mono text-amber-400">{e.registrations}</td>
                        </tr>
                      ))}
                      {execDetailedMatrix.length > 0 && (
                        <tr className="border-t border-slate-700 font-black text-[11px]">
                          <td className="py-2.5 pl-2 text-white">TOTAL</td>
                          <td className="py-2.5 text-center font-mono text-orange-400">{execDetailedMatrix.reduce((s, e) => s + e.calls, 0).toLocaleString()}</td>
                          <td className="py-2.5 text-center font-mono text-blue-400">{execDetailedMatrix.reduce((s, e) => s + e.followups, 0)}</td>
                          <td className="py-2.5 text-center font-mono text-emerald-400">{execDetailedMatrix.reduce((s, e) => s + e.siteVisits, 0)}</td>
                          <td className="py-2.5 text-center font-mono text-purple-400">{execDetailedMatrix.reduce((s, e) => s + e.bookings, 0)}</td>
                          <td className="py-2.5 text-center font-mono text-amber-400">{execDetailedMatrix.reduce((s, e) => s + e.registrations, 0)}</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          {activeTab === "leads" && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                  <h1 className="text-2xl font-black text-white tracking-tight">Active Lead Channels</h1>
                  <p className="text-xs text-slate-400 mt-0.5">Track property interaction vectors inside your regional territory.</p>
                </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-slate-900 border-b border-slate-800 text-slate-400 font-bold uppercase tracking-wider">
                        <th className="p-4 min-w-[150px]">Customer</th>
                        <th className="p-4 min-w-[150px]">Project</th>
                        <th className="p-4">Source</th>
                        <th className="p-4 min-w-[160px]">Assigned To</th>
                    ...
                    <tbody className="divide-y divide-slate-900 text-slate-300">
                      {processedLeads.length === 0 ? (
                        <tr><td colSpan={6} className="p-12 text-center text-slate-500 italic">No leads found.</td></tr>
                        ...
                        <tr key={l.id} className="hover:bg-slate-900/30 transition-all">
                          ...
                          <td className="p-4">
                            ...
                            <p className="text-[11px] text-slate-500 mt-0.5 font-mono">{l.dateCreated}</p>
                          </td>
                          ...
                          <td className="p-4">
                            {["Admin", "Manager"].includes(currentUser.role) ? (
                              <select value={l.assignedTo} onChange={e => requestOwnerAssignmentPopup(l.id, e.target.value)} className="bg-slate-900 border border-slate-800 text-slate-200 rounded px-2.5 py-1.5 text-xs font-semibold focus:outline-none focus:border-orange-500 cursor-pointer max-w-[170px]">
                                <option value="Unassigned">⚠️ Select Staff</option>
                                <optgroup label="Managers">{users.filter(u => u.role === "Manager").map(u => <option key={u.id} value={u.name}>{u.name} (Mgr)</option>)}</optgroup>
                                <optgroup label="Staff">{users.filter(u => ["Executive", "Telecaller"].includes(u.role)).map(u => <option key={u.id} value={u.name}>{u.name} ({u.role})</option>)}</optgroup>
                              </select>
                            ) : (
                          ...
                          <td className="p-4">
                            <div className="flex flex-col gap-1">
                              ...
                              {l.status === "Site Visit Planned" && l.siteVisitTentativeDate && <span className="text-[9px] text-purple-400 font-mono font-bold pl-1">📅 {l.siteVisitTentativeDate}</span>}
                            </div>
                          </td>
          ...
          {activeTab === "activity" && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                  <h1 className="text-2xl font-black text-white tracking-tight">Daily Activity Logs</h1>
                  <p className="text-xs text-slate-400 mt-0.5">Log daily calls, follow-ups, site visits, bookings and registrations per executive.</p>
                </div>
                <button onClick={() => setIsActivityLogModalOpen(true)} className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white font-black px-4 py-2 rounded-xl text-xs transition-colors shadow-md w-fit">
                  <Plus className="h-4 w-4" /> ADD ACTIVITY LOG
                </button>
              </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-900 text-slate-300">
                      {filteredActivityLogs.length === 0 ? (
                        <tr><td colSpan={12} className="p-10 text-center text-slate-500 italic">No activity logs found.</td></tr>
                      ) : filteredActivityLogs.map(l => (
                        <tr key={l.id} className="hover:bg-slate-900/30 transition-all">
                          <td className="p-3 font-mono text-slate-400 text-[11px]">{l.date}</td>
                          <td className="p-3 font-bold text-white">{l.executive}</td>
                          ...
                          <td className="p-3 text-center font-mono text-amber-400">{l.registration || ""}</td>
                          <td className="p-3 text-center font-mono text-rose-400">{l.cancellation || ""}</td>
                          <td className="p-3 text-[11px] text-slate-400 max-w-xs truncate" title={l.remark}>{l.remark}</td>
                        </tr>
                      ))}
                    </tbody>
                    {filteredActivityLogs.length > 0 && (
                      <tfoot className="bg-slate-900/50 border-t border-slate-800 font-black text-[11px]">
                        <tr>
                          <td className="p-3 text-white" colSpan={4}>TOTALS</td>
                          <td className="p-3 text-center font-mono text-white">{filteredActivityLogs.reduce((s, l) => s + (l.callsMade || 0), 0)}</td>
                          <td className="p-3"></td>
                          <td className="p-3 text-center font-mono text-blue-400">{filteredActivityLogs.reduce((s, l) => s + (l.followup || 0), 0)}</td>
                          <td className="p-3 text-center font-mono text-emerald-400">{filteredActivityLogs.reduce((s, l) => s + (l.siteVisit || 0), 0)}</td>
                          <td className="p-3 text-center font-mono text-purple-400">{filteredActivityLogs.reduce((s, l) => s + (l.booking || 0), 0)}</td>
                          <td className="p-3 text-center font-mono text-amber-400">{filteredActivityLogs.reduce((s, l) => s + (l.registration || 0), 0)}</td>
                          <td className="p-3 text-center font-mono text-rose-400">{filteredActivityLogs.reduce((s, l) => s + (l.cancellation || 0), 0)}</td>
                          <td className="p-3"></td>
                        </tr>
                      </tfoot>
                    )}
            ...
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Track property inventory capacities and structural workflows.</p>
                </div>
                {currentUser.role === "Admin" && (
                ...
                {visibleProjects.map(p => (
                  <div key={p.id} className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4 relative overflow-hidden shadow-xl">
                    <div className="flex justify-between items-start">
                      ...
                      {["Admin", "Manager"].includes(currentUser.role) ? (
                        <select value={p.status} onChange={e => handleProjectStatusChange(p.id, e.target.value)} className="bg-slate-900 border border-slate-800 rounded-lg px-2 py-1 text-[10px] font-black focus:outline-none cursor-pointer" style={{ color: PSC[p.status]?.text || "#FFF" }}>
                          {PROJECT_STATUSES.map(st => <option key={st} value={st}>{st}</option>)}
                        </select>
                      ) : (
                        <span className="text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider" style={{ backgroundColor: PSC[p.status]?.bg, color: PSC[p.status]?.text }}>{p.status}</span>
                      )}
                    </div>
                    <div className="bg-slate-900/60 rounded-xl p-3 border border-slate-800 text-xs space-y-2">
                      ...
                      <div className="pt-1.5 border-t border-slate-800">
                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 mb-1"><span>SOLD: {p.sold} / {p.units}</span><span className="font-mono text-orange-400">{Math.round((p.sold / p.units) * 100) || 0}%</span></div>
                        <div className="w-full bg-slate-950 h-1.5 rounded-full overflow-hidden"><div className="bg-gradient-to-r from-orange-600 to-amber-400 h-full transition-all duration-500" style={{ width: `${(p.sold / p.units) * 100}%` }}></div></div>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-8 pb-20">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start md:items-center gap-4 w-full">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Full Excel-style report with filters, summary matrix, executivewise, sourcewise & projectwise breakdowns.</p>
                </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-900">
                      {reportSummaryMatrix.metrics.map((metric, mi) => {
                        ...
                        const colors = ["text-orange-400", "text-blue-400", "text-emerald-400", "text-purple-400", "text-amber-400", "text-rose-400"];
                        return (
                    ...
                    <tbody className="divide-y divide-slate-900">
                      {[{ key: "calls", label: "Calls made", color: "text-orange-400" }, { key: "followups", label: "Followup", color: "text-blue-400" }, { key: "siteVisits", label: "Site visit", color: "text-emerald-400" }, { key: "bookings", label: "Booking", color: "text-purple-400" }, { key: "registrations", label: "Registration", color: "text-amber-400" }, { key: "cancellations", label: "Cancellation", color: "text-rose-400" }].map(row => (
                        <tr key={row.key} className="hover:bg-slate-900/30">
                          ...
                          <td className={`p-3 text-center font-mono font-black ${row.color}`}>{execDetailedMatrix.reduce((s, e) => s + (e[row.key] || 0), 0) || ""}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody className="divide-y divide-slate-900">
                      {sourcewiseMatrix.srcNames.map(src => (
                        <tr key={src} className="hover:bg-slate-900/30">
                          <td className="p-3 font-bold text-slate-300">{src}</td>
                          {sourcewiseMatrix.execNames.map(e => <td key={e} className="p-3 text-center font-mono text-slate-400">{sourcewiseMatrix.data[src]?.[e] || ""}</td>)}
                          <td className="p-3 text-center font-mono font-black text-white">{sourcewiseMatrix.srcTotals[src] || ""}</td>
                        </tr>
                      ))}
                      <tr className="border-t border-slate-700 font-black text-[11px]">
                        <td className="p-3 text-white">TOTAL</td>
                        {sourcewiseMatrix.execNames.map(e => <td key={e} className="p-3 text-center font-mono text-orange-400">{sourcewiseMatrix.srcNames.reduce((s, src) => s + (sourcewiseMatrix.data[src]?.[e] || 0), 0) || ""}</td>)}
                        <td className="p-3 text-center font-mono text-orange-400">{Object.values(sourcewiseMatrix.srcTotals).reduce((s, v) => s + v, 0) || ""}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-900">
                      {projectwiseMatrix.projNames.map(proj => (
                        <tr key={proj} className="hover:bg-slate-900/30">
                          <td className="p-3 font-bold text-orange-400">{proj}</td>
                          {projectwiseMatrix.execNames.map(e => <td key={e} className="p-3 text-center font-mono text-slate-400">{projectwiseMatrix.data[proj]?.[e] || ""}</td>)}
                          <td className="p-3 text-center font-mono font-black text-white">{projectwiseMatrix.projTotals[proj] || ""}</td>
                        </tr>
                      ))}
                      <tr className="border-t border-slate-700 font-black text-[11px]">
                        <td className="p-3 text-white">TOTAL</td>
                        {projectwiseMatrix.execNames.map(e => <td key={e} className="p-3 text-center font-mono text-amber-400">{projectwiseMatrix.projNames.reduce((s, proj) => s + (projectwiseMatrix.data[proj]?.[e] || 0), 0) || ""}</td>)}
                        <td className="p-3 text-center font-mono text-amber-400">{Object.values(projectwiseMatrix.projTotals).reduce((s, v) => s + v, 0) || ""}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-2xl h-fit">
                  ...
                  <form onSubmit={handleCreateUserSubmit} className="space-y-4 text-xs pt-1">
                    ...
                    <div className="space-y-1"><label className="text-slate-400 font-bold">Username Prefix *</label>
                      <div className="flex items-center bg-slate-900 border border-slate-800 rounded-xl overflow-hidden focus-within:border-orange-500">
                        <input type="text" required value={newUserForm.emailPrefix} onChange={e => setNewUserForm({ ...newUserForm, emailPrefix: e.target.value })} className="w-full bg-transparent p-2.5 text-slate-200 focus:outline-none text-right pr-1" placeholder="username" />
      ...
      {customPopup.isOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 w-full max-w-md rounded-2xl p-6 space-y-4 shadow-2xl text-center">
            <div className="h-12 w-12 bg-orange-500/10 text-orange-500 border border-orange-500/20 rounded-full flex items-center justify-center mx-auto"><ShieldAlert className="h-6 w-6" /></div>
            <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2.5 pt-2 text-xs uppercase font-black tracking-wider">
              <button onClick={() => setCustomPopup({ isOpen: false, leadId: null, targetValue: "", type: "status", title: "", message: "" })} className="w-full bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-white py-2.5 rounded-xl transition-all">✕ Cancel</button>
              <button onClick={confirmCustomPopupAction} className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 text-white py-2.5 rounded-xl transition-all shadow-md">✓ Confirm</button>
            </div>
          </div>
        </div>
      )}
      ...
      {toastNotification.isVisible && (
        <div className="fixed bottom-6 right-6 bg-slate-950 border border-emerald-500/40 text-emerald-400 font-bold px-4 py-3 rounded-xl shadow-2xl z-[110] flex items-center gap-2 text-xs uppercase tracking-wide">
          <div className="h-5 w-5 bg-emerald-500/10 rounded-full flex items-center justify-center"><Check className="h-3 w-3" /></div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl relative">
            <div className="flex justify-between items-center border-b border-slate-900 pb-3">
              <h2 className="text-base font-black text-white tracking-wide uppercase">Edit Profile</h2>
            ...
            <form onSubmit={handleUpdateUserSubmit} className="space-y-4 text-xs">
              ...
              <div className="space-y-1"><label className="text-slate-400 font-bold">Password *</label><input type="password" required value={editUserForm.pass} onChange={e => setEditUserForm({ ...editUserForm, pass: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-200 focus:outline-none" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-bold">Role</label><select value={editUserForm.role} onChange={e => setEditUserForm({ ...editUserForm, role: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-300 focus:outline-none">{INITIAL_ROLES.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
              ...
              <div className="space-y-1"><label className="text-slate-400 font-bold">Phone</label><input type="text" value={editUserForm.phone} onChange={e => setEditUserForm({ ...editUserForm, phone: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-200 font-mono" /></div>
              <button type="submit" className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 text-white font-black py-3 rounded-xl uppercase tracking-wider transition-all shadow-md mt-2">Update Profile</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedLead(null)}>
          <div className="bg-slate-950 border border-slate-800 w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-2xl p-6 space-y-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="border-b border-slate-900 pb-3 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
              ...
              <div className="flex items-center gap-4 flex-wrap sm:flex-nowrap">
                <div className="space-y-1">
                  ...
                  <select value={selectedLead.status} onChange={e => handleInlineMilestoneStatusChange(selectedLead.id, e.target.value)} className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs font-black focus:outline-none focus:border-orange-500 cursor-pointer min-w-[150px]" style={{ color: SC[selectedLead.status]?.text || "#FFF" }}>
                    {statuses.map(st => <option key={st} value={st}>{st}</option>)}
                  </select>
                </div>
                {["Admin", "Manager"].includes(currentUser.role) && (
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase tracking-wider text-slate-500 block">Assign To</label>
                    <select value={selectedLead.assignedTo} onChange={e => requestOwnerAssignmentPopup(selectedLead.id, e.target.value)} className="bg-slate-900 border border-slate-800 text-slate-100 rounded-xl px-3 py-1.5 text-xs font-black focus:outline-none focus:border-orange-500 cursor-pointer min-w-[180px]">
                      <option value="Unassigned">⚠️ Select Staff</option>
                      <optgroup label="Managers">{users.filter(u => u.role === "Manager").map(u => <option key={u.id} value={u.name}>{u.name} (Mgr)</option>)}</optgroup>
                      <optgroup label="Staff">{users.filter(u => ["Executive", "Telecaller"].includes(u.role)).map(u => <option key={u.id} value={u.name}>{u.name} ({u.role})</option>)}</optgroup>
                    </select>
            ...
            {selectedLead.status === "Site Visit Planned" && (
              <form onSubmit={commitTentativeWalkthroughPlan} className="bg-purple-950/40 p-4 border border-purple-500/30 rounded-xl space-y-3 text-xs">
                <div className="flex items-center gap-2 text-purple-400 font-black tracking-wide uppercase"><Calendar className="h-4 w-4" /> Set Tentative Site Visit Date</div>
                <div className="flex gap-2">
                  <input type="date" required min={TODAY_STR} value={tentativeWalkthroughDateInput} onChange={e => setTentativeWalkthroughDateInput(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-200 font-mono focus:outline-none focus:border-purple-500 cursor-pointer" />
                  <button type="submit" className="bg-purple-600 hover:bg-purple-700 text-white font-black px-5 rounded-xl uppercase tracking-wider transition-colors shadow-md">Save</button>
                </div>
              </form>
            )}
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-5">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2"><Clock className="h-4 w-4 text-orange-500" /> INTERACTION TIMELINE</h4>
                <span className="text-[10px] bg-slate-900 text-slate-400 px-2.5 py-0.5 rounded-full border border-slate-800 font-mono font-bold">{selectedLead.history?.length || 0} Entries</span>
              </div>
              <div className="relative max-h-[280px] overflow-y-auto pr-2 space-y-0 pt-2 pl-2">
                ...
                {selectedLead.history && selectedLead.history.length > 0 ? selectedLead.history.map((log, index) => {
                  ...
                  return (
                    <div key={index} className="flex gap-4 items-center pb-5 last:pb-2 group">
                      ...
                      <div className={`flex-1 bg-slate-900/50 border ${colorMap.border} p-3 rounded-xl space-y-1 hover:bg-slate-900 transition-all`}>
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1">
                          <h5 className="font-black text-white text-[11px] uppercase tracking-wide">By: <span className={colorMap.text}>{log.by}</span></h5>
                          <span className="text-[9px] font-bold text-slate-500 font-mono bg-slate-950 px-1.5 py-0.5 rounded">{log.date}</span>
                        </div>
                        <p className="text-slate-300 text-[11px] leading-relaxed font-medium pl-1.5 border-l border-slate-800">{log.action}</p>
                      </div>
                    </div>
                  );
            ...
            {!["Admin", "Manager"].includes(currentUser.role) && (
              <div className="space-y-5 text-xs">
                <div className="bg-slate-900 p-4 border border-slate-800 rounded-xl grid grid-cols-2 gap-4 font-semibold text-slate-300">
                  <div><p className="text-slate-500 text-[10px] font-bold uppercase">Project</p><p className="text-white mt-0.5 font-bold">{selectedLead.project}</p></div>
                  <div><p className="text-slate-500 text-[10px] font-bold uppercase">Budget</p><p className="text-emerald-400 mt-0.5 font-bold font-mono">₹{selectedLead.budget}L</p></div>
                  <div className="col-span-2"><p className="text-slate-500 text-[10px] font-bold uppercase">Notes</p><p className="text-slate-300 font-normal mt-0.5 italic">"{selectedLead.notes || 'None.'}"</p></div>
                </div>
                <form onSubmit={commitManualFollowUpReport} className="bg-slate-900/50 p-4 border border-slate-800 rounded-xl space-y-3">
                  <p className="text-[11px] font-black uppercase text-orange-400 tracking-wider">Log Follow-Up</p>
                  <div className="space-y-1"><label className="text-slate-400 font-medium text-[10px]">Notes *</label><textarea rows={2} required value={followUpNotes} onChange={e => setFollowUpNotes(e.target.value)} placeholder="Conversation summary..." className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2 text-slate-200 focus:outline-none" /></div>
                  <div className="space-y-1"><label className="text-slate-400 font-medium text-[10px]">Next Follow-up Date *</label><input type="date" required min={TODAY_STR} value={nextFollowUpDate} onChange={e => setNextFollowUpDate(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2 text-slate-200 focus:outline-none font-mono cursor-pointer" /></div>
                  <button type="submit" className="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-2 rounded-xl text-xs uppercase tracking-wider transition-all">Log Follow-up</button>
                </form>
                <div className="bg-slate-900/50 p-4 border border-slate-800 rounded-xl space-y-3">
                  <p className="text-[11px] font-black uppercase text-amber-400 tracking-wider">Log Site Visit</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1"><label className="text-slate-400 text-[10px]">Visit Date</label><input type="date" value={svDate} onChange={e => setSvDate(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded p-1.5 text-slate-200 font-mono" /></div>
                    <div className="space-y-1"><label className="text-slate-400 text-[10px]">Family / Co-Buyers</label><input type="text" value={svFamily} onChange={e => setSvFamily(e.target.value)} placeholder="Spouse, etc." className="w-full bg-slate-950 border border-slate-800 rounded p-1.5 text-slate-200" /></div>
                  </div>
                  <div className="space-y-1"><label className="text-slate-400 text-[10px]">Feedback *</label><textarea rows={1} value={svFeedback} onChange={e => setSvFeedback(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded p-2 text-slate-200 focus:outline-none" /></div>
                  <button type="button" onClick={commitSiteWalkthroughLog} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-1.5 rounded-xl uppercase tracking-wider">Confirm Site Visit</button>
                </div>
                <div className="bg-slate-900/50 p-4 border border-slate-800 rounded-xl space-y-3">
                  <p className="text-[11px] font-black uppercase text-emerald-400 tracking-wider">Secure Booking</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1"><label className="text-slate-400 text-[10px]">Unit *</label><input type="text" value={bkUnit} onChange={e => setBkUnit(e.target.value)} placeholder="e.g. Plot 42" className="w-full bg-slate-950 border border-slate-800 rounded p-1.5 text-slate-200" /></div>
                    <div className="space-y-1"><label className="text-slate-400 text-[10px]">Amount (₹) *</label><input type="number" value={bkAmount} onChange={e => setBkAmount(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded p-1.5 text-emerald-400 font-bold focus:outline-none" /></div>
                  </div>
                  <button type="button" onClick={commitFinancialBookingLog} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black py-1.5 rounded-xl uppercase tracking-wider">Confirm Booking</button>
                </div>
              </div>
            )}
          </div>
      ...
      {isLeadModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl relative">
            <div className="flex justify-between items-center border-b border-slate-900 pb-3">
              <h2 className="text-base font-black text-white tracking-wide uppercase">Add New Lead</h2>
            ...
            {duplicateConflictRecord && (
              <div className="absolute inset-x-6 top-16 bottom-6 bg-slate-950/95 border border-rose-500/40 rounded-xl p-5 z-20 flex flex-col justify-between space-y-3 backdrop-blur-md">
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-rose-400 font-black tracking-wide text-xs"><AlertTriangle className="h-5 w-5 animate-bounce" /> DUPLICATE DETECTED</div>
                  <p className="text-slate-400 text-[11px]">Phone <span className="text-white font-black font-mono bg-slate-900 px-1.5 py-0.5 rounded">{duplicateConflictRecord.phone}</span> already exists.</p>
                  <div className="bg-rose-500/10 border border-rose-500/20 rounded-lg p-3 space-y-1.5 text-xs">
                    <div className="flex justify-between"><span className="text-rose-300">Customer:</span><span className="text-white font-bold">{duplicateConflictRecord.name}</span></div>
                    <div className="flex justify-between"><span className="text-rose-300">Assigned:</span><span className="text-white font-bold">{duplicateConflictRecord.assignedTo}</span></div>
                  </div>
                </div>
      ...
      {isProjectModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b border-slate-900 pb-3">
              <h2 className="text-base font-black text-white tracking-wide uppercase">Add New Project</h2>
            ...
            <form onSubmit={handleCreateProject} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1"><label className="text-slate-400 font-semibold">Type</label><select value={newProjectForm.type} onChange={e => setNewProjectForm({ ...newProjectForm, type: e.target.value })} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">{PROJECT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                <div className="space-y-1"><label className="text-slate-400 font-semibold">Price (₹L)</label><input type="number" value={newProjectForm.price} onChange={e => setNewProjectForm({ ...newProjectForm, price: e.target.value })} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200 font-mono" /></div>
                <div className="space-y-1"><label className="text-slate-400 font-semibold">Status</label><select value={newProjectForm.status} onChange={e => setNewProjectForm({ ...newProjectForm, status: e.target.value })} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-300 focus:outline-none">{PROJECT_STATUSES.map(st => <option key={st} value={st}>{st}</option>)}</select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-semibold">Total Units</label><input type="number" value={newProjectForm.units} onChange={e => setNewProjectForm({ ...newProjectForm, units: e.target.value })} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200" /></div>
                <div className="space-y-1"><label className="text-slate-400 font-semibold">Sold Units</label><input type="number" value={newProjectForm.sold} onChange={e => setNewProjectForm({ ...newProjectForm, sold: e.target.value })} className="w-full bg-slate-900 border border-slate-800 p-2.5 rounded-xl text-slate-200" /></div>
              </div>
              <button type="submit" className="w-full bg-gradient-to-r from-orange-600 to-orange-500 text-white font-black py-3 rounded-xl uppercase tracking-wider shadow-lg">Add Project</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isActivityLogModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full max-w-2xl space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-900 pb-3">
              <h2 className="text-base font-black text-white tracking-wide uppercase">Add Daily Activity Log</h2>
              <button onClick={() => setIsActivityLogModalOpen(false)} className="text-slate-500 hover:text-white">✕</button>
            </div>
            <form onSubmit={handleCreateActivityLog} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-bold">Date *</label><input type="date" required value={newActivityForm.date} onChange={e => setNewActivityForm({ ...newActivityForm, date: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-200 font-mono focus:outline-none focus:border-orange-500" /></div>
                <div className="space-y-1">
                  <label className="text-slate-400 font-bold">Executive/Telecaller *</label>
                  {["Admin", "Manager"].includes(currentUser.role) ? (
                    <select value={newActivityForm.executive} onChange={e => setNewActivityForm({ ...newActivityForm, executive: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-300 focus:outline-none" required>
                      <option value="">Select Executive</option>
                      {users.filter(u => ["Executive", "Telecaller", "Manager"].includes(u.role)).map(u => <option key={u.id} value={u.name}>{u.name} ({u.role})</option>)}
                    </select>
                  ) : (
                    <input type="text" value={currentUser.name} disabled className="w-full bg-slate-900/50 border border-slate-800 rounded-xl p-2.5 text-slate-400 opacity-70" />
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-bold">Project *</label><select required value={newActivityForm.project} onChange={e => setNewActivityForm({ ...newActivityForm, project: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-300 focus:outline-none">{projects.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}</select></div>
                <div className="space-y-1"><label className="text-slate-400 font-bold">Source *</label><select required value={newActivityForm.source} onChange={e => setNewActivityForm({ ...newActivityForm, source: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-300 focus:outline-none">{sources.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-bold">Calls Made</label><input type="number" min="0" value={newActivityForm.callsMade} onChange={e => setNewActivityForm({ ...newActivityForm, callsMade: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-200 font-mono focus:outline-none" /></div>
              ...
              <div className="space-y-1"><label className="text-slate-400 font-bold">Remark</label><textarea rows={2} value={newActivityForm.remark} onChange={e => setNewActivityForm({ ...newActivityForm, remark: e.target.value })} placeholder="Notes about calls, follow-ups, or client feedback..." className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-slate-200 focus:outline-none focus:border-orange-500" /></div>
              <button type="submit" className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 text-white font-black py-3 rounded-xl uppercase tracking-wider shadow-lg">Save Activity Log</button>
            </form>
          </div>
        </div>
      )}
