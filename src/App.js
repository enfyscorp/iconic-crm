...
export default function App() {
  ...
  const exportSelectedRangeReport = (formatType) => {
    ...
    const renderPdfDonutChart = (items, colorOffset = 0) => {
      const filtered = items.filter(item => (Number(item.value) || 0) > 0).slice(0,8);
      const total = filtered.reduce((sum,item)=>sum+(Number(item.value)||0),0);
      if(!total)return `<div class="empty-chart">No data</div>`;
      const radius = 52;
      const circumference = 2 * Math.PI * radius;
      let offset = 0;
      const rings = filtered.map((item,idx)=>{
        const fraction=(Number(item.value)||0)/total;
        const dash=fraction*circumference;
        const circle=`<circle cx="75" cy="75" r="${radius}" fill="none" stroke="${PIE_COLORS[(idx+colorOffset)%PIE_COLORS.length]}" stroke-width="26" stroke-dasharray="${dash.toFixed(2)} ${(circumference-dash).toFixed(2)}" stroke-dashoffset="${(-offset).toFixed(2)}" transform="rotate(-90 75 75)"/>`;
        offset+=dash;
        return circle;
      }).join("");
      const legend=filtered.map((item,idx)=>`<span><i style="background:${PIE_COLORS[(idx+colorOffset)%PIE_COLORS.length]}"></i>${esc(item.name)}: ${formatReportValue(item.value)}</span>`).join("");
      return `<div class="donut-wrap"><svg viewBox="0 0 150 150" role="img" aria-label="Share chart">${rings}<circle cx="75" cy="75" r="34" fill="#fff"/><text x="75" y="72" text-anchor="middle" font-size="11" fill="#475569">TOTAL</text><text x="75" y="90" text-anchor="middle" font-size="18" font-weight="700" fill="#0f172a">${total}</text></svg><div class="chart-list">${legend}</div></div>`;
    };
    const renderPdfExecutiveBars = (items) => {
      const filtered=items.filter(item=>(Number(item.calls)||0)>0||(Number(item.booking)||0)>0).slice(0,10);
      if(!filtered.length)return `<div class="empty-chart">No data</div>`;
      const maxValue=Math.max(...filtered.flatMap(item=>[Number(item.calls)||0,Number(item.booking)||0]),1);
      const rows=filtered.map(item=>`<div class="exec-row"><span>${esc(item.name)}</span><div><b class="calls-bar" style="width:${Math.max(((Number(item.calls)||0)/maxValue)*100,2)}%">${formatReportValue(item.calls)}</b><b class="booking-bar" style="width:${Math.max(((Number(item.booking)||0)/maxValue)*100,2)}%">${formatReportValue(item.booking)}</b></div></div>`).join("");
      return `<div class="exec-bars">${rows}</div><div class="legend"><span><i style="background:#f97316"></i>Calls</span><span><i style="background:#a855f7"></i>Booking</span></div>`;
    };
    ...
    const dashboardHtml = dashboard ? (() => {
      ...
      return `<section class="report-cover"><div class="cover-head"><div><h1>${esc(activeRangeReport.title)}</h1><p>${esc(pdfRangeLongLabel)}</p></div><img src="${DESAM_LOGO_ASSET}" alt="Logo"/></div><div class="metric-grid">${renderPdfKpiSection("Given Period", `${pdfDateLabel(reportStartDate)} to ${pdfDateLabel(reportEndDate)}`, dashboard.totals)}${renderPdfKpiSection("Today", pdfDateLabel(TODAY_STR), dashboard.todayTotals || dashboard.totals)}</div><div class="dash-grid"><div class="dash-card"><h3>Enquiry Trend</h3><div class="trend">${trend}</div></div><div class="dash-card"><h3>Enquiry, Calls & Booking Line</h3>${renderPdfLineChart(trendItems)}</div><div class="dash-card"><h3>Source Enquiry Share</h3>${renderPdfDonutChart(dashboard.sourceMix)}</div><div class="dash-card"><h3>Project Enquiry Share</h3>${renderPdfDonutChart(dashboard.projectMix,4)}</div><div class="dash-card wide-card"><h3>Executive Calls Vs Booking</h3>${renderPdfExecutiveBars(dashboard.executiveBars)}</div></div></section>`;
    })() : "";
    ...
    win.document.write(`<html><head><title>${esc(fileStem)}</title><style>*{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}body{font-family:Arial,sans-serif;padding:24px;color:#111;background:#fff}.pdf-toolbar{position:sticky;top:0;z-index:10;display:flex;justify-content:flex-end;gap:8px;padding:10px;margin:-24px -24px 18px;background:#0f172a}.pdf-toolbar button{border:0;border-radius:6px;padding:9px 14px;color:#fff;font-weight:700;cursor:pointer}.save-btn{background:#dc2626}.print-btn{background:#2563eb}.close-btn{background:#475569}h1{font-size:20px}h2{font-size:15px;margin:24px 0 2px;text-align:center}h2 span{display:block;font-size:12px;margin-top:3px}.report-date{text-align:center;font-size:11px;margin:0 0 8px;color:#475569}table{width:100%;border-collapse:collapse;margin-top:8px;font-size:11px;table-layout:fixed}th,td{border:1px solid #111;padding:5px;text-align:center;vertical-align:middle;width:90px}th{background:#f1f5f9;font-weight:700}.cover-head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #111;padding-bottom:10px}.cover-head img{height:38px;object-fit:contain}.metric-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:14px 0}.metric-section{border:1px solid #cbd5e1;padding:10px;background:#f8fafc}.metric-section h3{margin:0;text-align:center;font-size:13px}.metric-section p{margin:3px 0 8px;text-align:center;font-size:10px;color:#475569}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}.box{border:1px solid #ddd;padding:7px;text-align:center;background:#fff;font-size:9px}.box b{display:block;font-size:15px}.dash-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.dash-card{border:1px solid #ddd;padding:10px;min-height:180px;break-inside:avoid}.wide-card{grid-column:1/-1}.dash-card h3{text-align:center;margin:0 0 10px;font-size:13px}.trend{height:160px;display:flex;align-items:flex-end;gap:6px;border-left:1px solid #ddd;border-bottom:1px solid #ddd;padding:8px}.trend-col{display:flex;flex-direction:column;align-items:center;gap:4px;flex:1}.trend-col strong{font-size:9px;font-weight:700}.trend-col span{width:100%;display:block;border-radius:3px 3px 0 0}.trend-col em{font-size:8px;font-style:normal}.line-wrap svg{width:100%;height:160px}.donut-wrap{display:grid;grid-template-columns:145px 1fr;gap:8px;align-items:center}.donut-wrap svg{width:145px;height:145px}.chart-list{display:flex;flex-direction:column;gap:5px;font-size:9px}.chart-list span{display:flex;align-items:center;gap:5px}.chart-list i,.legend i{width:9px;height:9px;display:inline-block;border-radius:99px;flex:none}.legend{display:flex;justify-content:center;gap:10px;font-size:9px}.legend span{display:inline-flex;align-items:center;gap:4px}.exec-bars{display:flex;flex-direction:column;gap:6px}.exec-row{display:grid;grid-template-columns:110px 1fr;gap:8px;align-items:center;font-size:9px}.exec-row>div{display:flex;flex-direction:column;gap:2px}.exec-row b{display:block;color:#fff;min-width:18px;padding:2px 4px;text-align:right;border-radius:2px}.calls-bar{background:#f97316}.booking-bar{background:#a855f7}.empty-chart{height:140px;display:flex;align-items:center;justify-content:center;color:#64748b}.report-cover{page-break-after:always}.table-head{display:flex;justify-content:flex-end;margin-top:12px}.table-head img{height:30px}@page{size:A4 landscape;margin:10mm}@media print{.pdf-toolbar{display:none!important}body{padding:0}table{page-break-inside:auto}tr{page-break-inside:avoid}.report-cover{min-height:auto}.dash-card{break-inside:avoid}}</style></head><body><div class="pdf-toolbar"><button class="save-btn" onclick="window.print()">Save PDF</button><button class="print-btn" onclick="window.print()">Print</button><button class="close-btn" onclick="window.close()">Close</button></div>${dashboardHtml || `<div class="table-head"><img src="${DESAM_LOGO_ASSET}" alt="Logo"/></div><h1>${esc(activeRangeReport.title)}</h1><p style="text-align:center">${esc(pdfRangeLongLabel)}</p><div class="kpis"><div class="box">Enquiry<b>${formatReportValue(selectedRangeReportTotals.source.enquiry)}</b></div><div class="box">Calls<b>${formatReportValue(selectedRangeReportTotals.people.calls)}</b></div><div class="box">Followup<b>${formatReportValue(selectedRangeReportTotals.people.followup)}</b></div><div class="box">SV Planned<b>${formatReportValue(selectedRangeReportTotals.people.siteVisitPlanned)}</b></div><div class="box">SV Done<b>${formatReportValue(selectedRangeReportTotals.people.siteVisitDone)}</b></div><div class="box">Booking<b>${formatReportValue(selectedRangeReportTotals.people.booking)}</b></div><div class="box">Conversion %<b>${formatReportValue(`${selectedRangeReportTotals.source.percentage}%`)}</b></div><div class="box">Cancellation<b>${formatReportValue(selectedRangeReportTotals.people.cancellation)}</b></div></div>`}${reportTables}</body></html>`);
    win.document.close();
