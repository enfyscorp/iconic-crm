...
export default function App() {
  ...
  const exportSelectedRangeReport = (formatType) => {
    ...
    const dashboardHtml = dashboard ? (() => {
      ...
      const funnel = dashboard.funnel.map(item=>`<div class="funnel-step" style="width:${Math.max((item.value/maxFunnel)*100,12)}%;background:${item.color}"><span>${esc(item.name)}</span><b>${formatReportValue(item.value)}</b></div>`).join("");
